...
const Orders = () => {
  ...
  useEffect(() => {
    fetch(`https://fast-headland-05721.herokuapp.com/orders`)
      .then((res) => res.json())
      .then((data) => {
        setOrders(data);
        setLoading(false);
  ...
  const handleStatusChange = (id, status) => {
    ...
    fetch("https://fast-headland-05721.herokuapp.com/updateOrderStatus", {
      method: "put",
  ...
  const deletion = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://fast-headland-05721.herokuapp.com/placeorder/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
